...
from c_elegans_constants import ADULT_WORM_AGE, SINGLE_DEVELOPMENTAL_AGE, THREE_DEVELOPMENTAL_AGES, \
    FULL_DEVELOPMENTAL_AGES
...
def train_reciprocity_dependence_model_distributed_single_stage_full_dataset(num_artificial_types=8):
    ...
    all_data_sets_path = os.path.join(cur_path, "CElegansData", "InferredTypes", "connectomes",
...
def train_reciprocity_dependence_model_distributed_syn_list(num_artificial_types, dev_stages, all_data_sets_path,
    ...
    out_dir_path_spls = os.path.join(cur_path, "SavedOutputs", "ReciprocalModel", "DyadsSplit", "S+s",
    ...
    search_spl_per_type_reciprocity_dependence_model_distributed_syn_list(params_path, gamma, out_dir_path_spls,
                                                                          ...
                                                                          FULL_DEVELOPMENTAL_AGES,
                                                                          all_data_sets_path)
    out_dir_path_likelihoods = os.path.join(cur_path, "SavedOutputs", "ReciprocalModel", "DyadsSplit",
    ...
    for stage in dev_stages:
        ...
        dev_ages[idx] = FULL_DEVELOPMENTAL_AGES[cur_stage]
        idx += 1
...
def run_reciprocity_dependence_model_distributed_single_dev_stage(spls_path, smi, beta, gamma,
...
def run_reciprocity_dependence_model_distributed_across_gammas_single_dev_stage(gamma_range):
    cur_path = os.getcwd()
    ...
    for gamma in gamma_range:
        ...
        if not os.path.exists(out_dir_path):
            try:
                ...
                pass
        run_reciprocity_dependence_model_distributed_single_dev_stage(spls_path, smi, beta, gamma,
                                                                      out_dir_path,
                                                                      num_artificial_types=num_artificial_types)


def train_single_epoch_dyads_split():
    num_artificial_types = 8
    all_data_sets_path = os.path.join("InferredTypes", "synapses_lists",
                                      f"{num_artificial_types}_types")
    dev_stages = [7]
...
def train_three_epochs_dyads_split():
    num_artificial_types = 8
    all_data_sets_path = os.path.join("InferredTypes", "synapses_lists", f"{num_artificial_types}_types")
    dev_stages = [1, 5, 7]
    train_reciprocity_dependence_model_distributed_syn_list(num_artificial_types, dev_stages, all_data_sets_path,
                                                            "ThreeDevStages")


def calc_custom_epochs_model_dyads_dists_across_splits_syn_list(dev_stages_str, developmental_ages, smi_value=None):
    cur_path = os.getcwd()
    func_id = int(os.environ['LSB_JOBINDEX'])
    ...
    test_synapses_list_path = os.path.join(cur_path, "CElegansData", "InferredTypes",
                                           "synapses_lists", f"{num_types}_types", f'split{split}', "test",
                                           "Dataset7.pkl")
    train_synapses_list_path = os.path.join(cur_path, "CElegansData", "InferredTypes",
                                            "synapses_lists", f"{num_types}_types", f'split{split}', "train",
                                            "Dataset7.pkl")

    smi, beta, _ = find_max_likelihood_full_model(
        os.path.join(cur_path, "SavedOutputs", "ReciprocalModel", "DyadsSplit", "likelihoods", dev_stages_str,
                     f"split{split}"),
        smi_value=smi_value)
    spls_file_name = f"spls_smi{smi:.5f}_beta{beta:.5f}.pkl"
    spls_path = (
        os.path.join(cur_path, "SavedOutputs", "ReciprocalModel", "DyadsSplit", "S+s", dev_stages_str,
                             f"split{split}",
                     spls_file_name))
    with open(spls_path, 'rb') as f:
        spls = pickle.load(f)
    out_dir_path_dyads_dist_test = (
        os.path.join(cur_path, "SavedOutputs", "ReciprocalModel", "DyadsSplit",
...
def calc_dyads_dists_single_epoch_dyads_split():
...
def calc_dyads_dists_three_epochs_dyads_split(smi_value=None):
    dev_stages_str = "ThreeDevStages"
    developmental_ages = THREE_DEVELOPMENTAL_AGES
    calc_custom_epochs_model_dyads_dists_across_splits_syn_list(dev_stages_str, developmental_ages, smi_value=smi_value)


def main():
    train_reciprocity_dependence_model_distributed_single_stage_full_dataset()
    run_reciprocity_dependence_model_distributed_across_gammas_single_dev_stage(range(2, 10))

    train_single_epoch_dyads_split()
    train_three_epochs_dyads_split()

    save_max_like_params_per_split_single_epoch()
    save_max_like_params_per_split_three_epochs()

    calc_dyads_dists_single_epoch_dyads_split()
    calc_dyads_dists_three_epochs_dyads_split()
    calc_dyads_dists_three_epochs_dyads_split(smi_value=0)
